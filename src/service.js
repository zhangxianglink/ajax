...
var server = http.createServer(function(request, response){
  ...
  console.log('有人发请求过来啦！路径（带查询参数）为：' + pathWithQuery)
   ...
   if(path === '/index.html'){
    ...
    const jsonObj = JSON.parse(fs.readFileSync('public/page.json'))
    const liArr = jsonObj.map(e => `<li>${e.id}</li>`).join('')
    response.write(htmlStr.replace('{{page}}',`<ul id = 'pageUl'>${liArr}</ul>`))
    response.end()
